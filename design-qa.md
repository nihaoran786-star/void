...
## 目录页与侧边栏重构（2026-08-18）

- 设计真值：`design-lab/redesign-handoff/` 的 01–04 效果图与业主实机反馈。
  颜色不参考效果图，全部取自 Void 现有主题令牌，由业主明确指示。
- 验证方式：Windows 桌面应用（浅色主题）实机截图，Per-Monitor-V2 DPI 感知、
  DWM 物理边界捕获；关键对齐用像素测量而非目测。工作用截图在记录结论后删除。

### 已核对并修复的实机差异

- **连接器页与另外两页比例不一致**：连接器页套在设置页布局里，页面内边距、
  最大宽度与两侧滚动条预留槽叠在共享内容框之上。改为由共享内容框单独决定列宽。
  量值：上栏细线原为 `425 → 1762`，技能页为 `410 → 1777`；修复后两者同为
  `410 → 1777`。
- **技能市场整页空白**：目录卡片上的 `content-visibility: auto` 让浏览器跳过
  卡片内容渲染，只剩按内在尺寸撑出的空盒。数据其实已加载（标题计数为 20）。
  去掉该声明后内容正常；同时给加载占位加上呼吸，避免"加载中"与"内容没出来"同形。
- **四个目录页上栏不一致**：智能体页在上栏之外另有一条切换条。四个表面改为共用
  同一个 `DirectoryTopBar`。
- **一条栏里两个同等重量的黑丸**：目录切换丸保持深底白字，筛选丸改为浅底深字。
- **技能图标千篇一律**：匹配用了整条运行时身份，工作区目录名含 `codex`，几乎所有
  技能都命中"代码"图标。改为只用显示名与身份最后一段，并给关键词加词边界。
- **侧边栏图标不成列**：定义统一图标中线。量值：展开态收起按钮与助理、自动化、
  定制的图标中心同为 `55`；收起态为 `49–50`；底部按钮与行图标同为 `55`。
- **分区分割线**：标题原为「小字 + 横贯细线 + 数字」，细线读成断裂分割线，已移除。
- **钉住的分区标题透明**：下方行会从标题底下透出。标题改为承载导航区自身材质。
- **分区各自滚动导致空洞**：曾把侧边栏切成每分区一个滚动框，实机出现收起分区被推到
  底部、中间大片空白、一栏两个滚动条。已回退为单一滚动区 + 分区标题 sticky。
- **工作区展开状态丢失**：折叠状态原为组件局部 state，重排序、列表刷新、切换场景
  都会重新展开，且激活工作区会强制展开。改为按工作区 id 存于
  `navWorkspaceFoldStore`。实机验证：收起 `void-source` → 切到助理 → 切到自动化，
  仍为收起。

### 按业主决定保留的与效果图的差异

- AI 员工卡不显示运行计时：目录里没有逐员工的运行时长事实来源，不凭空构造。
- 连接器筛选丸为六枚而非效果图的两枚：后四枚是真实可用的状态筛选，以真实为主。
- 技能没有"启用/停用"开关：Void 的技能只有"被同名技能覆盖"这种状态，未新增机能。

### 门禁

类型检查、构建、lint（0 错误）、i18n 契约与三语审计、全量 2886 项 Web 测试、
边界检查、主题视觉契约均通过。`check:theme-colors` 与 `check:repo-hygiene`
的失败为仓库既有项，数值与改动前一致，不由本次改动引入。
