...
A fully custom, branded installer for Void — built with **Tauri 2 + React** for maximum UI flexibility.
...
```bash
npx tauri dev -- -- --uninstall "D:\\tmp\\void-uninstall-test"
```
...
```text
src-tauri/target/release/void-installer.exe
```

Fast build output:

```text
src-tauri/target/release-fast/void-installer.exe
```
...
Place the built Void application files in `src-tauri/payload/` before building the installer. The build script handles this automatically.
During `cargo build`, the payload directory is packed into an embedded zip inside `void-installer.exe`.
...
- name: Upload Installer
  ...
  with:
    name: Void-Installer-Exe
    path: Void-Installer/src-tauri/target/release/void-installer.exe
```
