/**
 * void Installer build script.
 *
 * Steps:
 * 1. Build void main app (optional).
...
const VOID_ROOT = path.resolve(ROOT, "..");
const PAYLOAD_DIR = path.join(ROOT, "src-tauri", "payload");
const LEGACY_BRAND_LOWER = "bit" + "fun";
...
function run(cmd, cwd = ROOT) {
  log(`> ${cmd}`);
  try {
    execSync(cmd, { cwd, stdio: "inherit", env: buildEnv() });
  } catch (_e) {
...
function buildEnv() {
  return {
    ...process.env,
    RUSTFLAGS: appendRustFlag(
      process.env.RUSTFLAGS || "",
      `--remap-path-prefix=${VOID_ROOT}=void-src`
    ),
  };
}

function appendRustFlag(current, flag) {
  return current.includes(flag) ? current : `${current} ${flag}`.trim();
}
...
Options:
  ...
  --skip-app-build       Skip building main void app
...
function shouldCopySiblingRuntimeFile(fileName, appExeBaseName) {
  ...
  if (lower.includes(LEGACY_BRAND_LOWER)) return false;
  if (
    lower.endsWith(".pdb") ||
    lower.endsWith(".d") ||
    lower.endsWith(".exp") ||
...
function getCandidateAppExePaths(mode) {
  ...
  for (const profile of preferredProfiles) {
    candidates.push(
      ...
      path.join(VOID_ROOT, "target", profile, "void-desktop.exe")
    );
  }
...
function hasLegacyBrandPath(value) {
  return String(value).toLowerCase().includes(LEGACY_BRAND_LOWER);
}
...
// Step 1: Build main void app.
if (!skipAppBuild) {
  log("Step 1: Building void main application...");
  run(getMainAppBuildCommand(buildMode), VOID_ROOT);
} else {
...
if (!appExePath && STRICT_PAYLOAD_VALIDATION) {
  error(
    "Could not find built void executable for payload. Build the desktop app first or run with --dev for local debug."
  );
}
...
if (appExePath) {
  ...
  const manifest = {
    ...
    mode: buildMode,
    sourceExe: path.basename(appExePath),
    files: [],
  };

  const destExe = path.join(PAYLOAD_DIR, "void-desktop.exe");
  ...
  if (STRICT_PAYLOAD_VALIDATION && exeSize < MIN_APP_EXE_BYTES) {
    error(
      `void-desktop.exe in payload is unexpectedly small (${exeSize} bytes). Refusing to continue.`
    );
  }
  ...
  if (
    STRICT_PAYLOAD_VALIDATION &&
    manifest.files.some((file) => hasLegacyBrandPath(file.path))
  ) {
    error("Payload manifest contains legacy-branded files. Refusing to build installer.");
  }
} else {
...
const installerTargetProfile = isDev
  ...
  : buildMode === "fast"
    ? "release-fast"
    : "release";
cleanupGeneratedSidecars(path.join(ROOT, "src-tauri", "target", installerTargetProfile), [
  "void_installer.pdb",
  "void-installer.d",
]);
...
function cleanupGeneratedSidecars(targetDir, fileNames) {
  for (const fileName of fileNames) {
    ...
    log(`Removed generated sidecar: ${filePath}`);
  }
}
