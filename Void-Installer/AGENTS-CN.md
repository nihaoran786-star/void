...
本文件适用于 `Void-Installer`。仓库级规则请看顶层 `AGENTS.md`。

## 这里最重要的内容

`Void-Installer` 是独立的 Tauri + React 应用，不属于主 Cargo workspace。
...
pnpm --dir Void-Installer run build            # React build / 复现 CI
pnpm --dir Void-Installer run installer:build  # 仅打包场景
```
...
pnpm --dir Void-Installer run type-check                            # 前端 i18n/runtime
cargo check --manifest-path Void-Installer/src-tauri/Cargo.toml      # Tauri/Rust 改动
```
...
如果修改了卸载流程，还需要验证 `Void-Installer/README.md` 中描述的卸载模式入口。
