...
pub const TAURI_MANUFACTURER: &str = "void Team";
/// Matches Tauri NSIS `PRODUCTNAME` (`productName`).
pub const TAURI_PRODUCT_NAME: &str = "void";
...
/// Data read from the product uninstall key (Tauri NSIS / this installer).
#[derive(Debug, Clone)]
...
pub fn remove_context_menu() -> Result<()> {
    let hkcu = RegKey::predef(HKEY_CURRENT_USER);
    let _ = hkcu.delete_subkey_all(r"Software\Classes\Directory\Background\shell\void");
    let _ = hkcu.delete_subkey_all(r"Software\Classes\Directory\shell\void");
    Ok(())
}
