...
pub fn get_default_install_path() -> String {
    ...
    base.join("void").to_string_lossy().to_string()
}
...
/// Detect existing void install (Tauri NSIS or this installer) via Add/Remove Programs registry.
#[tauri::command]
...
fn run_installation(
    ...
    payload_candidates: Vec<PayloadCandidate>,
    mut progress: impl FnMut(&str, u32, &str),
) -> Result<(), String> {
    ...
    let result: Result<(), String> = (|| {
        ...
        progress("prepare", 5, "Creating installation directory...");
        std::fs::create_dir_all(&install_path)
            .map_err(|e| format!("Failed to create directory: {}", e))?;

        // Step 2: Extract / copy application files
        progress("extract", 15, "Extracting application files...");
        ...
        progress("extract", 50, "Files extracted successfully");
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            progress("registry", 60, "Registering application...");
            ...
            if options.desktop_shortcut {
                progress("shortcuts", 70, "Creating desktop shortcut...");
            ...
            if options.start_menu {
                progress("shortcuts", 75, "Creating Start Menu entry...");
        ...
        // Step 4: Save first-launch language preference for void app.
        progress("config", 92, "Applying startup preferences...");
...
pub async fn start_installation(window: Window, options: InstallOptions) -> Result<(), String> {
    ...
    let payload_candidates = build_payload_candidates(resource_dir, &exe_dir);
    run_installation(&options, payload_candidates, |step, percent, message| {
        emit_progress(&window, step, percent, message);
    })
}

/// Non-interactive installer entry for release verification.
///
/// This is intentionally only reachable from the process command line so the
/// normal React installer flow stays unchanged.
pub fn qa_install_and_exit(install_path: Option<String>) -> Result<(), String> {
    let install_path = install_path
        .map(|path| path.trim().to_string())
        .filter(|path| !path.is_empty())
        .unwrap_or_else(get_default_install_path);
    ...
    let exe_dir = std::env::current_exe()
        ...
        .unwrap_or_else(|| Path::new("."))
        .to_path_buf();
    let payload_candidates = build_payload_candidates(None, &exe_dir);
    run_installation(&options, payload_candidates, |step, percent, message| {
        println!("[qa-install] {percent:>3}% {step}: {message}");
    })
}

/// Uninstall void (for the uninstaller companion).
#[tauri::command]
...
fn schedule_windows_self_uninstall_cleanup(uninstall_exe_path: &Path) -> Result<(), String> {
    ...
    let script_path = temp_dir.join(format!("void-uninstall-{}.cmd", pid));
    let log_path = temp_dir.join(format!("void-uninstall-cleanup-{}.log", pid));
...
set "TARGET_DIR=%~dp1"
if "%TARGET%"=="" exit /b 2
if "%LOG%"=="" set "LOG=%TEMP%\void-uninstall-cleanup.log"
...
fn append_uninstall_runtime_log(message: &str) {
    let ts = std::time::SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    let log_path = std::env::temp_dir().join("void-uninstall-runtime.log");
...
pub fn launch_application(install_path: String) -> Result<(), String> {
    let exe = if cfg!(target_os = "windows") {
        ...
        PathBuf::from(&install_path).join("void")
    } else {
        PathBuf::from(&install_path).join("void")
    };
    ...
    create_windows_silent_command(&exe)
        ...
        .spawn()
        .map_err(|e| format!("Failed to launch void: {}", e))?;
...
pub async fn test_model_config_connection(
    ...
    let ai_client = void_ai_adapters::AIClient::new(ai_config);
...
/// Stored `request_url` aligned with settings `resolveRequestUrl` (no void_core).
...
/// Actual install root is always under a `void` directory: `{user choice}/void`.
/// If the user already chose a path whose last segment is `void`, do not append again.
fn with_void_install_subdir(path: PathBuf) -> PathBuf {
    let already_void = path
        .file_name()
        .and_then(|n| n.to_str())
        .map(|s| s.eq_ignore_ascii_case("void"))
        .unwrap_or(false);
    if already_void {
        path
    } else {
...
fn prepare_install_target(requested_path: &Path) -> Result<PathBuf, String> {
    ...
    let install_path = with_void_install_subdir(requested_path.to_path_buf());
    ...
    if install_path.exists() {
        ...
        if directory_has_entries(&install_path)? && !install_path.join(MAIN_APP_EXE).exists() {
            return Err(format!(
                "{}directory_must_be_empty_or_void",
    ...
    let test_file = writable_dir.join(".void_install_test");
...
fn ensure_app_config_path() -> Result<PathBuf, String> {
    ...
    std::fs::create_dir_all(&config_root)
        .map_err(|e| format!("Failed to create void config directory: {}", e))?;
