...
pub fn run() {
    ...
    if let Some(idx) = args.iter().position(|arg| arg == "--qa-install-and-exit") {
        let install_path = args.get(idx + 1).cloned();
        if let Err(err) = commands::qa_install_and_exit(install_path) {
            eprintln!("QA install failed: {err}");
            std::process::exit(1);
        }
        std::process::exit(0);
    }
