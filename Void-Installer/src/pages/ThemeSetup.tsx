...
export function ThemeSetup({ options, setOptions, onLaunch, onClose }: ThemeSetupProps) {
  ...
  const lightPreview = findInstallerThemeById('void-light');
  const darkPreview = findInstallerThemeById('void-dark');
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      setFinishError(typeof err === 'string' ? err : (err as Error)?.message || 'Failed to launch Void');
    } finally {
