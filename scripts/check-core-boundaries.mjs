...
const optionalDependencyFeatureOwnerRules = [
  ...
  },
  {
    ...
    dependencies: [
      ...
      { depName: 'void-runtime-ports', ownerFeatures: ['remote-connect'] },
      { depName: 'void-services-core', ownerFeatures: ['git', 'mcp'] },
    ...
    patterns: [
      {
        regex: /\btokio::fs::/,
        message:
          'core filesystem service must not own async local filesystem IO; use void-services-core filesystem primitives',
      },
      {
        regex: /\bstd::fs::/,
        message:
          'core filesystem service must not own sync local filesystem IO; use void-services-core filesystem primitives',
      },
      {
        regex: /\bignore::WalkBuilder\b/,
        message:
          'core filesystem service must not own local file walking/search implementation; use void-services-core filesystem primitives',
      },
      {
        regex: /\bsha2::/,
        message:
          'core filesystem service must not own editor-sync hashing implementation; use void-services-core filesystem primitives',
      },
      {
        regex: /\bbase64::/,
        message:
          'core filesystem service must not own binary file encoding implementation; use void-services-core filesystem primitives',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    patterns: [
      ...
      },
      {
        ...
        message:
          'core tool pipeline must not own provider-neutral assistant result rendering; use void-agent-tools',
      },
      {
        regex: /\bconst TOOL_ERROR_ARGUMENTS_PREVIEW_BYTES\b/,
        message:
          'core tool pipeline must not own tool error argument preview limits; use void-agent-tools',
      },
      {
        regex: /\bfn truncate_arguments_preview\b/,
        message:
          'core tool pipeline must not own tool error argument preview rendering; use void-agent-tools',
      },
      {
        regex: /\bfn truncate_raw_arguments_preview\b/,
        message:
          'core tool pipeline must not own raw tool argument preview rendering; use void-agent-tools',
      },
      {
        regex: /\bconst USER_STEERING_INTERRUPTED_MESSAGE\b/,
        message:
          'core tool pipeline must not own steering-interrupted result presentation; use void-agent-tools',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core subagent runtime must not redefine DelegationPolicy; use void-runtime-ports',
      },
      {
        regex: /\benum\s+SubagentContextMode\b/,
        message:
          'core subagent runtime must not redefine SubagentContextMode; use void-runtime-ports',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core coordinator must not redefine DialogTriggerSource; use void-runtime-ports',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core scheduler must not redefine DialogQueuePriority; use void-runtime-ports',
      },
      {
        regex: /\bstruct\s+DialogSubmissionPolicy\b/,
        message:
          'core scheduler must not redefine DialogSubmissionPolicy; use void-runtime-ports',
      },
      {
        regex: /\benum\s+DialogSubmitOutcome\b/,
        message:
          'core scheduler must not redefine DialogSubmitOutcome; use void-runtime-ports',
      },
      {
        regex: /\bstruct\s+AgentSessionReplyRoute\b/,
        message:
          'core scheduler must not redefine AgentSessionReplyRoute; use void-runtime-ports',
      },
      {
        regex: /\benum\s+DialogSteerOutcome\b/,
        message:
          'core scheduler must not redefine DialogSteerOutcome; use void-runtime-ports',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core round preempt runtime must not redefine DialogRoundPreemptSource; use void-runtime-ports',
      },
      {
        regex: /\bstruct\s+RoundInjection\b/,
        message:
          'core round preempt runtime must not redefine RoundInjection; use void-runtime-ports',
      },
      {
        regex: /\btrait\s+DialogRoundInjectionSource\b/,
        message:
          'core round preempt runtime must not redefine DialogRoundInjectionSource; use void-runtime-ports',
      },
      {
        regex: /\benum\s+RoundInjectionKind\b/,
        message:
          'core round preempt runtime must not redefine RoundInjectionKind; use void-runtime-ports',
      },
      {
        regex: /\benum\s+RoundInjectionTarget\b/,
        message:
          'core round preempt runtime must not redefine RoundInjectionTarget; use void-runtime-ports',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message: 'core goal mode types must not redefine GOAL_MODE_METADATA_KEY; use void-runtime-ports',
      },
      {
        regex: /\bconst\s+MAX_GOAL_CONTINUATIONS\b/,
        message: 'core goal mode types must not redefine MAX_GOAL_CONTINUATIONS; use void-runtime-ports',
      },
      {
        regex: /\bconst\s+MAX_CONTEXT_SUMMARY_CHARS\b/,
        message: 'core goal mode types must not redefine MAX_CONTEXT_SUMMARY_CHARS; use void-runtime-ports',
      },
      {
        regex: /\bstruct\s+GoalModeInitialGoal\b/,
        message: 'core goal mode types must not redefine GoalModeInitialGoal; use void-runtime-ports',
      },
      {
        regex: /\bstruct\s+GoalModeState\b/,
        message: 'core goal mode types must not redefine GoalModeState; use void-runtime-ports',
      },
      {
        regex: /\bstruct\s+GoalGenerationResult\b/,
        message: 'core goal mode types must not redefine GoalGenerationResult; use void-runtime-ports',
      },
      {
        regex: /\bstruct\s+GoalVerificationResult\b/,
        message: 'core goal mode types must not redefine GoalVerificationResult; use void-runtime-ports',
      },
      {
        regex: /\bstruct\s+GoalActivationResult\b/,
        message: 'core goal mode types must not redefine GoalActivationResult; use void-runtime-ports',
      },
      {
        regex: /\bstruct\s+GoalContinuationPlan\b/,
        message: 'core goal mode types must not redefine GoalContinuationPlan; use void-runtime-ports',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message: 'core message model must not redefine CompressionContract; use void-runtime-ports',
      },
      {
        regex: /\bstruct\s+CompressionContractItem\b/,
        message: 'core message model must not redefine CompressionContractItem; use void-runtime-ports',
      },
      {
        regex: /\bfn\s+render_contract_items\b/,
        message: 'core message model must not own compression contract rendering; use void-runtime-ports',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message: 'core workspace manager must not redefine RelatedPath; use void-runtime-ports',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core file read-state runtime must delegate pure freshness normalization to void-agent-tools',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core tool result storage must delegate pure preview generation to void-agent-tools',
      },
      {
        regex: /\bfn\s+build_persisted_output_message\b/,
        message:
          'core tool result storage must delegate persisted-output rendering to void-agent-tools',
      },
      {
        regex: /\bfn\s+select_candidates_to_persist\b/,
        message:
          'core tool result storage must delegate round-budget selection to void-agent-tools',
      },
      {
        regex: /\bstruct\s+ToolResultStoragePolicy\b/,
        message:
          'core tool result storage must use the provider-neutral storage policy from void-agent-tools',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'DelegationPolicy and SubagentContextMode must be imported from void-runtime-ports, not the core compatibility re-export',
      },
    ],
  },
  {
...
const requiredContentRules = [
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /pub\(crate\) use void_runtime_ports::\{DelegationPolicy, SubagentContextMode\};/,
  ...
  },
  {
    ...
    patterns: [
      ...
      {
        regex: /pub use void_runtime_ports::DialogTriggerSource;/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex:
          /pub use void_runtime_ports::\{[\s\S]*AgentSessionReplyRoute[\s\S]*DialogQueuePriority[\s\S]*DialogSteerOutcome[\s\S]*DialogSubmissionPolicy[\s\S]*DialogSubmitOutcome[\s\S]*\};/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex:
          /pub use void_runtime_ports::\{[\s\S]*DialogRoundInjectionSource[\s\S]*DialogRoundPreemptSource[\s\S]*RoundInjection[\s\S]*RoundInjectionKind[\s\S]*RoundInjectionTarget[\s\S]*\};/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex:
          /pub use void_runtime_ports::\{[\s\S]*GoalActivationResult[\s\S]*GoalContinuationPlan[\s\S]*GoalGenerationResult[\s\S]*GoalModeInitialGoal[\s\S]*GoalModeState[\s\S]*GoalVerificationResult[\s\S]*GOAL_MODE_METADATA_KEY[\s\S]*MAX_CONTEXT_SUMMARY_CHARS[\s\S]*MAX_GOAL_CONTINUATIONS[\s\S]*\};/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /pub use void_runtime_ports::\{CompressionContract, CompressionContractItem\};/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /pub use void_runtime_ports::RelatedPath;/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /\bpub use void_product_domains::miniapp::runtime::\{/,
...
function runManifestParserSelfTest() {
  ...
  for (const dep of ['void-runtime-ports', 'git2', 'notify', 'rmcp']) {
