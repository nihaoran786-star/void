...
async function main() {
  ...
  appendRustFlag(`--remap-path-prefix=${ROOT}=void-src`);
  ...
  if (r.status === 0) {
    cleanupGeneratedSidecars(join(ROOT, 'target', tauriBuildProfile(forward)), [
      'void_desktop.pdb',
      'void-desktop.d',
    ]);
  }
...
function appendRustFlag(flag) {
  const current = process.env.RUSTFLAGS || '';
  process.env.RUSTFLAGS = current.includes(flag) ? current : `${current} ${flag}`.trim();
}

function tauriBuildProfile(args) {
...
function cleanupGeneratedSidecars(targetDir, fileNames) {
  for (const fileName of fileNames) {
    ...
    unlinkSync(filePath);
    console.log(`[tauri-build] Removed generated sidecar: ${filePath}`);
  }
}
...
function prepareTauriConfig(baseConfigPath, { desktopDir, flashgrepBinary }) {
  ...
  if (enabled) {
    ...
    const endpoint =
      ...
      'https://github.com/void/void/releases/latest/download/latest.json';
