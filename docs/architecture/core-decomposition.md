...
## High-risk migration registry（风险登记，不是迁移许可）

下面登记的是上游值得研究、但当前不能直接复刻的 owner/runtime 迁移候选。登记只说明
“需要先补证据再决定是否拆 issue”，不表示 `Cargo.toml`、crate layout、manager owner、
app entrypoint 或 concrete runtime 已获准迁移。

| Candidate | Upstream evidence | Current Void owner | Target seam idea | Risk class | Protected surfaces | Required evidence before code | Forbidden in registry issue | Next issue required | Status |
|---|---|---|---|---|---|---|---|---|---|
| Concrete runtime owners to services | `213299206` | `void-core` still owns MiniApp/function-agent IO, remote SSH concrete managers, deep runtime bindings, product runtime assembly. | Identify pure helper seams only; keep concrete runtime owner unchanged until a per-owner issue exists. | `concrete-runtime` | Multi-agent/subagent, Flow Chat tool cards, AI media, AI short-drama, MiniApp/product-domain runtime, remote SSH, terminal remote history, MCP/provider boundaries. | MiniApp builtin/storage/worker/host-dispatch snapshots; remote SSH manager/remote_fs/remote_terminal/remote_exec equivalence tests; remote terminal history `status/source/error`; AI media/short-drama boundary tests. | Manager moves, feature graph edits, product runtime assembly changes, broad services crate migration. | Yes: per concrete owner migration issue. | `registered-only` |
| Terminal exec runtime port | `98f0f4113` | `terminal-core` owns PTY/session/replay; desktop `terminal_api.rs` adapts Tauri DTOs; Web `TerminalService` / `useTerminal` own replay/live consumption. | Study a narrow exec-runtime seam after terminal replay/input/ack snapshots are stable. | `concrete-runtime` | Terminal replay/history, live output ack, Flow Chat terminal tool card, desktop API, CLI exec, Computer Use terminal/GVim input paths. | Exec command local run/stdin/control/completion/exit lifecycle tests; replay flat+structured compatibility; hook-level ack tests; Flow Chat terminal card snapshot; Computer Use terminal input checks. | Replacing terminal execution path, moving PTY/session owner, changing replay/live ordering, changing app bootstrap. | Yes: terminal runtime-port implementation issue. | `registered-only` |
| Remote exec through runtime port | `bceded210` | Remote SSH manager, remote terminal/fs/search/SFTP, desktop `agentic_api` / `tool_api`, and scheduler queue integration remain current owners. | Treat remote exec port as an adapter seam only after remote identity, lifecycle, and queue semantics have snapshots. | `concrete-runtime` | Remote SSH, remote workspace identity, terminal unsupported-history semantics, Flow Chat tool results, multi-agent execution queues, desktop APIs. | Remote command request/response snapshots; remote unsupported replay vs empty history tests; env snapshot/cancel/control/stdin semantics; multi-agent no cross-session leakage; desktop API `status/source/error` checks. | Moving remote SSH manager, changing network lifecycle, changing scheduler submit/cancel semantics, changing desktop command DTOs. | Yes: remote exec runtime-port issue. | `registered-only` |
| Provider HTTP owners into services | `96cea08ca` | `void-ai-adapters` owns AI provider HTTP/SSE; service integrations own only focused non-provider helpers unless separately accepted. | Separate web/review/browser HTTP helpers from AI provider transport owner; do not merge provider ownership. | `service-helper` plus `concrete-runtime` risk | Provider adapters, OpenAI/Anthropic/Gemini/Responses stream parsing, AI media APIMart, image understanding, Flow Chat provider errors, MCP Streamable HTTP. | Provider HTTP/SSE owner guard; OpenAI content-part parser regression; connection-test typed errors; SSE drop cancellation; review/web tool HTTP snapshots; APIMart and MCP Streamable HTTP owner checks. | Moving AI provider transport out of `void-ai-adapters`, changing provider wire conversion outside adapters, changing media/image provider path. | Yes: provider/service boundary issue. | `registered-only` |
| MCP server runtime state to services | `8d69e5733` | Core MCP manager/product tool runtime still owns catalog exposure, runtime state integration, and model-visible tool catalog behavior. | Extract only policy/cache helpers after catalog/runtime-state snapshots prove behavior equivalence. | `service-helper` with `concrete-runtime` risk | MCP manager lifecycle, Streamable HTTP timeout, readonly/dynamic provider metadata, tool catalog snapshot, tool result storage, Flow Chat MCP tool cards, AI media/short-drama tool isolation. | MCP catalog cache snapshot; registry add/remove/reconnect lifecycle tests; Streamable HTTP timeout contracts; readonly manifest/provider metadata; large-output storage behavior; typed tool approval/rejection outcomes. | Moving manager lifecycle, changing tool registry snapshot, changing product tool materialization, changing model-visible tool results. | Yes: MCP runtime-state migration issue. | `registered-only` |
| Product tool runtime owner split | `8d69e5733`, `ISSUE-1150F` | `void-core::agentic::tools::product_runtime` materializes concrete tools, `ToolUseContext`, registry snapshot and GetToolSpec runtime. | Define a product-tool runtime seam only after manifest/tool-result/tool-card snapshots exist. | `concrete-runtime` | Tool manifest, collapsed-tool unlock state, readonly policy, MCP/tools catalog, Flow Chat tool cards, AI media/short-drama tools, subagent tool execution. | Manifest snapshot; registry snapshot; readonly and collapsed-tool tests; assistant-visible result snapshots; Flow Chat tool-card compatibility tests. | Moving concrete tools or `ToolUseContext`, changing registry snapshot shape, changing assistant-visible tool result shape. | Yes: product tool runtime issue. | `registered-only` |
| Product-full / app assembly profile split | `a7fc6dcd1`, `ISSUE-1180B` | Desktop and CLI currently depend on `void-core product-full`; server and relay remain existing app surfaces. | Consider service availability or delivery profile only after product requirement and feature graph evidence. | `feature-graph` plus `concrete-runtime` risk | Desktop, CLI, ACP, server, relay, multi-agent/subagent, `/goal`, Flow Chat, AI media, AI short-drama, terminal, Computer Use, provider, MCP, installer. | `cargo metadata` / feature graph snapshots; app entrypoint feature checks; product capability snapshots; startup smoke for desktop/CLI if behavior changes. | New runtime profile, app entrypoint rewrites, `product-full` weakening, installer/release behavior changes. | Yes: product assembly/profile issue. | `registered-only` |

登记表禁止使用 `approved`、`ready to implement`、“直接同步上游”或“迁移到目标 crate”
