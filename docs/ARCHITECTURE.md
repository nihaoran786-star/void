...
- A future `ViewImage` tool is not a replacement for `AnalyzeImage`. It may only prepare a readonly tool-result image attachment from an explicitly resolved workspace image path after manifest exposure, provider image-attachment support, and local/remote path resolution are proven in separate issues. It must use `ToolImageAttachment` for image payloads, return explicit `status/source/error` plus image metadata, and must not add provider-specific wire logic to core or read files from UI/media/short-drama services.
