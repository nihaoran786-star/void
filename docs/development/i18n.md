...
What this is: the step-by-step rules for adding or changing translated strings.
Read it whenever you touch `src/web-ui/src/locales/**`. Architecture rationale
lives in `docs/architecture/i18n.md`.
...
- Do not import `src/web-ui/src/locales` from `src/mobile-web`,
  `Void-Installer`, Rust crates, or server apps.
...
pnpm --dir Void-Installer run type-check                            # installer frontend i18n runtime
cargo check --manifest-path Void-Installer/src-tauri/Cargo.toml      # installer Tauri/Rust changes
cargo test -p void-core i18n -- --nocapture                         # backend i18n runtime changes
```
