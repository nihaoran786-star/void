...
Priority: P1
Status: Done
Goal: Make the `ISSUE-1190A1` turn-navigation e2e type preflight reproducible from the root pnpm workspace lockfile.
Allowed files: `pnpm-lock.yaml`, migration docs.
Forbidden files: Flow Chat runtime components, `FlowChatStore`, session restore/deferred hydration APIs, backend/Tauri/Rust, AI media, AI short-drama, subagent/BTW internals, terminal, provider, Canvas, generated version files, package dependency changes beyond the lockfile entry already implied by `tests/e2e/package.json`.
Acceptance:
- Root `pnpm-lock.yaml` records the `tests/e2e` `@wdio/globals` dependency declared in `tests/e2e/package.json`.
- `pnpm install --lockfile-only --ignore-scripts` reports the workspace lockfile is up to date.
- `pnpm install --frozen-lockfile --ignore-scripts` passes.
- Focused turn-navigation type preflight still passes.
Result:
- Committed the root pnpm lockfile update generated by the existing `tests/e2e/package.json` dependency change.
- Verified lockfile-only install was already up to date and frozen install passed without running build scripts.
- Preserved all Flow Chat runtime, session restore, AI media, AI short-drama, terminal, provider, Canvas, and generated version files.
