...
## DEC-130: CLI Rich Model Rounds Are Rebuilt In Session Persistence Only

Decision: `ISSUE-1190G` accepts upstream commit `6fd87c11a` as a targeted session persistence fix. `ConversationCoordinator` may pass `ExecutionResult.new_messages` into `SessionManager::complete_dialog_turn`, and `SessionManager` may rebuild missing persisted `ModelRoundData` from those messages when no assistant text was already persisted for the turn. The rebuilt data must stay within the current Void session schema: text items, thinking items, tool items, tool results, stable `round_index`, and stable `order_index`. It must not introduce upstream-only fields, change event ABI, change Flow Chat rendering, or create a second session/turn owner.

Reason: CLI execution already produces rich assistant/tool messages, but the local completion path could fall back to final text only, losing thinking and tool result history for Desktop and other session restore surfaces. Keeping the conversion inside `SessionManager` preserves `Session -> DialogTurn -> ModelRound` ownership while avoiding broad Flow Chat, Canvas, terminal, provider, AI media, or AI short-drama coupling.

## DEC-131: Session Restore Replays Persisted Tool Calls Through The Existing Message Model

Decision: `ISSUE-1191A` extends the existing `SessionManager::build_messages_from_turns` conversion so persisted model rounds replay as runtime `Message` values with assistant thinking/text/tool calls followed by matching tool result messages. This remains a restore-time conversion from `DialogTurnData.model_rounds` into the current `MessageContent::Mixed` and `MessageContent::ToolResult` model. It does not add new session schema fields, change event ABI, change CLI output rendering, or move restore ownership into Flow Chat, terminal, providers, Canvas, AI media, or AI short-drama modules.

Reason: `ISSUE-1190G` made CLI rich model rounds durable, but restore/replay still needed to preserve tool calls and tool results for long-running sessions and follow-up turns. Keeping the replay conversion in `SessionManager` uses the existing state model and avoids duplicating model-round interpretation in UI or CLI entrypoints.

## DEC-132: CLI Truecolor Defaults Are Preset-Derived

Decision: `ISSUE-1191B` accepts upstream commit `912cd7561` as a governance pattern, not a direct rename or merge. Void CLI `Theme::dark()` and `Theme::light()` must derive their truecolor defaults from local `void-dark` and `void-light` builtin preset JSON through the same resolved-token mapping used by custom opencode themes. `apply_opencode_theme_json` remains the partial-theme overlay boundary: missing custom fields inherit from the chosen base theme. ANSI16 and monochrome themes keep their explicit non-truecolor mappings.

Reason: Maintaining a second Rust RGB fallback table made CLI theme defaults drift from preset governance and kept hidden color debt outside preset ownership. Deriving defaults from Void presets makes CLI runtime theming and CLI color audit agree while preserving preset IDs, visual token files, Web UI ThemeService, installer/mobile themes, Canvas, Flow Chat, AI media, and AI short-drama boundaries.

## DEC-133: Terminal Core Owns Local History Status And Source

Decision: `ISSUE-1191C` moves the local terminal history availability facts into terminal-core `GetHistoryResponse` by adding explicit `TerminalHistoryStatus` and `TerminalHistorySource` DTO values. Local core history responses return `ready/local`; desktop remains the adapter boundary that can classify known remote sessions as `unsupported/remote` with `remote_history_unsupported`. Web UI continues to consume explicit `historyStatus/historySource` and must not infer source or availability from empty replay data.

Reason: `ISSUE-1120D` hardened desktop/Web history contracts, but core still returned only replay payload and dimensions, leaving desktop to synthesize local status/source. Moving local facts into terminal-core gives the owner of local PTY replay the status contract while preserving remote terminal ownership, Web terminal behavior, runtime-port boundaries, Flow Chat, MCP, provider, AI media, and AI short-drama separation.

## DEC-134: Remote File CRUD Carries Connection Context Through Existing Boundaries

Decision: `ISSUE-1192A` accepts upstream `1ab4d323f` as a targeted remote workspace file-operation fix. Web `WorkspaceAPI` may carry an optional `remoteConnectionId` for file create/delete/rename requests, desktop command DTOs may deserialize it, and `path_target` remains the only adapter that resolves local versus remote file operations. `RemoteWorkspaceRegistry` must not let a mismatched preferred connection id discard a unique path match; it may only use the hint to disambiguate multiple matching registrations.

Reason: Remote workspaces can share path shapes such as `/` across SSH connections, while local file operations must remain path-based and backward compatible. Carrying an explicit connection hint through existing service boundaries fixes remote CRUD routing without moving remote SSH logic into `FilesPanel`, command entrypoints, Flow Chat, AI media, AI short-drama, terminal, provider, Canvas, installer, or upstream directory layouts.

## DEC-135: Recent Upstream Closeout Requires Explicit Disposition, Not Opportunistic Implementation

Decision: `ISSUE-1194A` classifies the latest `upstream-bitfun/main@1cce339c` gaps without implementing them. SSH connection dialog search/fill-form UX and SSH config parsing fallbacks are deferred as a remote-workspace UX/parser family that needs its own connection state model, parser contract, explicit `status/source/error`, and focused tests. Legacy Rust theme-schema retirement is folded into theme governance and can only proceed through a future schema/token compatibility issue. OpenCode extension architecture documentation remains a reference under the tool/plugin runtime migration gate from `DEC-114`; it does not approve event ABI, plugin runtime, or owner movement.

Reason: These upstream changes are real but cross different product boundaries. Treating them as quick fixes during goal closeout would mix SSH UI state, parser/runtime behavior, theme schema compatibility, and tool/plugin architecture into unrelated migration slices. Recording their disposition preserves closeout integrity while protecting current remote workspace CRUD, ThemeService, CLI theme runtime, tool runtime, Flow Chat, AI media, AI short-drama, terminal, provider, and generated-file boundaries.
