...
Decision: `ISSUE-1140D1` rejects direct upstream `view_image` copying and splits any Void-native `ViewImage` implementation into separate manifest/readonly exposure, provider image-attachment capability, and workspace path/image-processing slices. `ViewImage` must remain distinct from `AnalyzeImage`: `AnalyzeImage` performs model image understanding, while `ViewImage` may only return a readonly tool-result image attachment plus explicit metadata after the gates pass.
