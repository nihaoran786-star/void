...
无限画布面板在 classic 与 minimal 两套主题下都渲染为同一组暗色——它是看片台，
与图片查看器同理。实现上新增画布专属 `--canvas-*` token 定义在
`.infinite-canvas-panel` 上，minimal 覆盖层只接管字体与间距、不再重映射颜色，
因此浅色主题下该面板不跟随全局配色。这是业主定稿的取舍，不是遗漏。
