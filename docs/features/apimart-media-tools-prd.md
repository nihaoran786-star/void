...
BitFun now has a Media session mode, but it cannot yet execute real image, video, upload, text-to-speech, or transcription requests. The user has provided the first batch of APIMart API examples. All media requests use `https://api.apimart.ai` and a single bearer token, but the models have different parameters, supported input combinations, polling behavior, result shapes, and cost risks.
