...
> Non-goal: this is not a media editor, media generator, full task monitor, APIMart surface, BrowserPanel extension, or backend session feature.
...
The module is a workspace media browser. It does not edit media, generate media, call APIMart, or read chat message text. It indexes and presents files from the workspace, and it may render generation-in-progress placeholders from normalized workspace media job metadata or explicit media lifecycle refresh signals.
...
18. As a user, I want the gallery to react to media-generation lifecycle changes and still keep manual refresh available, so that generated assets appear without losing explicit user control.
...
25. As a maintainer, I want media generation, APIMart polling, and gallery rendering connected only through normalized media job and refresh state, so that this gallery cannot regress generation workflows.
...
- Manual refresh remains available, but media tool lifecycle signals and persisted media job records should refresh or populate the gallery without requiring a manual click. File-watch auto-refresh is intentionally deferred.
...
- Media generation cards should not be parsed as UI text by the gallery. If chat-side tool state contributes to live refresh, it must go through a small conversion hook that writes normalized workspace-media refresh state.
- Generated media saved under workspace asset directories should naturally appear because they are files in the workspace. Generation-in-progress placeholders should come from persisted media job records or normalized lifecycle refresh state, not from gallery-specific inference.
...
- Generating media.
- Polling APIMart or media-generation tasks from the gallery.
- Reading chat message text or private chat card presentation state from the gallery.
- Changing visible media generation card behavior beyond emitting normalized workspace-media refresh signals.
