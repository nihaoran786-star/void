...
使用 WebDriverIO + void 内嵌 WebDriver 进行 void 项目的端到端测试完整指南。
...
void E2E 测试专注于**用户旅程**和**关键路径**，确保桌面应用从用户角度正常工作。我们使用分层测试方法来平衡覆盖率和执行速度。
...
**核心原理**: E2E 只使用 `target/debug/void-desktop.exe`。如果 debug 二进制不存在，应直接失败，而不是回退到 `release`。
...
# 用 debug 模式运行测试，void 会在进程内启动 WebDriver
...
**症状**: `Application not found at target/debug/void-desktop.exe`
