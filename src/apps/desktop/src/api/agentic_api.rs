...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct UpdateSessionGoalRequest {
    pub session_id: String,
    pub action: String,
    #[serde(default)]
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct UpdateSessionGoalResponse {
    pub success: bool,
    pub status: String,
    pub active: bool,
    pub goal_text: Option<String>,
    ...
    pub continuation_display_message: Option<String>,
    pub continuation_metadata: Option<serde_json::Value>,
}

fn goal_mode_status_wire(status: GoalModeStatus) -> &'static str {
    match status {
        GoalModeStatus::Active => "active",
        GoalModeStatus::Paused => "paused",
        GoalModeStatus::Blocked => "blocked",
        GoalModeStatus::UsageLimited => "usage-limited",
        GoalModeStatus::Complete => "complete",
    }
}
...
pub async fn update_session_goal(
...
) -> Result<UpdateSessionGoalResponse, String> {
    let session_id = request.session_id.trim();
    if session_id.is_empty() {
        return Err("session_id is required".to_string());
    }
        ...
        .is_none()
    {
        ...
        coordinator
            .restore_session(&effective, session_id)
            .await
            .map_err(|e| format!("Failed to restore session before updating goal mode: {e}"))?;
    }

    let action = match request.action.trim().to_ascii_lowercase().as_str() {
        "pause" => GoalModeUpdateAction::Pause,
        "resume" => GoalModeUpdateAction::Resume,
        "clear" => GoalModeUpdateAction::Clear,
    ...
    let result = coordinator
        .update_session_goal(session_id.to_string(), action)
        .await
        .map_err(|error| error.to_string())?;
    ...
    let status = result
        .state
        .as_ref()
        .map(|state| goal_mode_status_wire(state.status).to_string())
    ...
    let active = result
        ...
        .unwrap_or(false);
    let goal_text = result.state.as_ref().map(|state| state.goal_text.clone());
    let token_budget = result.state.as_ref().and_then(|state| state.token_budget);
    let tokens_used = result
        .state
        .as_ref()
        .map(|state| state.tokens_used)
        .unwrap_or(0);
    let continuation_plan = result.continuation_plan;

    Ok(UpdateSessionGoalResponse {
        success: true,
        status,
        active,
        goal_text,
        ...
        continuation_message: continuation_plan
            .as_ref()
            .map(|plan| plan.wrapped_message.clone()),
        continuation_display_message: continuation_plan
            .as_ref()
            .map(|plan| plan.display_message.clone()),
        continuation_metadata: continuation_plan.map(|plan| plan.user_message_metadata),
    })
}
...
pub async fn steer_dialog_turn(
    ...
    let steering_id = match outcome {
        void_core::agentic::coordination::DialogSteerOutcome::Buffered { steering_id, .. } => {
            steering_id
        }
    };
...
mod tests {
    ...
    fn goal_mode_status_wire_uses_stable_kebab_case_for_usage_limited() {
        assert_eq!(goal_mode_status_wire(GoalModeStatus::Active), "active");
        assert_eq!(
            goal_mode_status_wire(GoalModeStatus::UsageLimited),
            "usage-limited"
        );
    }

    #[test]
    fn update_session_goal_response_serializes_goal_accounting_fields() {
        let response = UpdateSessionGoalResponse {
            success: true,
            status: "usage-limited".to_string(),
            active: false,
            goal_text: Some("Ship feature".to_string()),
            token_budget: Some(1000),
            tokens_used: 1000,
            display_message: "Goal token budget reached".to_string(),
            continuation_message: None,
            continuation_display_message: None,
            continuation_metadata: None,
        };

        let json = serde_json::to_value(response).expect("serialize response");

        assert_eq!(json["status"], "usage-limited");
        assert_eq!(json["tokenBudget"], 1000);
        assert_eq!(json["tokensUsed"], 1000);
    }
