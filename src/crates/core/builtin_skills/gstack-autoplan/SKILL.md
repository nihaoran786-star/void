...
This is qualitatively different from taste decisions. When void and outside-voice sub-agent both
...
All prompts sent to outside-voice sub-agent (via `void Task outside-voice dispatch` or `void Task outside-voice review`) MUST be prefixed with
...
echo "RESTORE_PATH=$HOME/.void/team/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Discover design docs: `ls -t $HOME/.void/team/projects/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
- Detect DX scope: grep the plan for developer-facing terms (API, endpoint, REST,
  ...
  package, npm, pip, import, require, SKILL.md, skill template, void, MCP, agent,
  ...
  on top of) or if an AI agent is the primary user (OpenClaw actions, void skills,
  MCP servers).
...
- Test plan: generate artifact at `$HOME/.void/team/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- [ ] Test plan artifact written to disk at $HOME/.void/team/projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
