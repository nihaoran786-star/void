...
When this skill is invoked by Void Team Mode, this skill supplies the engineering-manager review lens. Use existing Task sub-agents for independent architecture and evidence gathering, then synthesize decisions in the main Team session.
...
Write to `$HOME/.void/team/projects/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md`:
...
Analyze the plan's implementation steps for parallel execution opportunities. This helps the user split work across Void Task sub-agents, git worktrees, or separate workspaces when the workstreams are genuinely independent.
