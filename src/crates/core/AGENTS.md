...
- MiniApp built-in bundle/hash/marker seed-plan and marker wire helpers may
  live in `void-product-domains`. MiniApp create/update/draft/apply pure state
...
- Remote-connect wire/tracker/dialog and cancel orchestration plus portable
  ...
  assembly helpers may live in `void-services-integrations`; keep
...
- Generic local filesystem operations, tree/search, listing, and filesystem DTOs
  live in `void-services-core::filesystem`. Core may keep compatibility
  re-exports, remote workspace overlay, `voidError` mapping, MiniApp
...
- Provider-neutral tool path resolution, effective absolute-path checks,
  ...
  text may live in `void-agent-tools`; file guidance markers, file-read
  ...
  `voidError` category mapping, and `ToolUseContext`
...
- Keep `void-core/product-full` explicitly wired to the current owner feature
