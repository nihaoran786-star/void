...
impl PathManager {
    ...
    /// - Linux: ~/.config/void/
    fn get_user_config_root() -> VoidResult<PathBuf> {
        let config_dir = dirs::config_dir()
            .ok_or_else(|| VoidError::config("Failed to get config directory".to_string()))?;

        Ok(config_dir.join("void"))
    }

    /// Get assistant home root directory: ~/.void/
    pub fn void_home_dir(&self) -> PathBuf {
        if let Some(path) = &self.void_home_override {
            return path.clone();
        }
        dirs::home_dir()
    ...
    /// Get the legacy assistant workspace base directory: ~/.void/
    ///
    ...
    pub fn legacy_assistant_workspace_base_dir(&self, override_root: Option<&Path>) -> PathBuf {
        override_root
            ...
            .unwrap_or_else(|| self.void_home_dir())
    }

    /// Get assistant workspace base directory: ~/.void/personal_assistant/
    ///
    /// `override_root` is reserved for future user customization.
    ...
    /// Get the legacy default assistant workspace directory: ~/.void/workspace
    pub fn legacy_default_assistant_workspace_dir(&self, override_root: Option<&Path>) -> PathBuf {
        self.legacy_assistant_workspace_base_dir(override_root)
            .join("workspace")
    }

    /// Get the default assistant workspace directory: ~/.void/personal_assistant/workspace
    ...
    /// Get a legacy named assistant workspace directory: ~/.void/workspace-<id>
    ...
    /// Get a named assistant workspace directory: ~/.void/personal_assistant/workspace-<id>
    ...
    /// True if `path` is this machine's Void **assistant** workspace directory.
    ///
    /// Used so remote-workspace registry (especially roots like `/`) does not
    /// mis-classify client paths such as `/Users/.../.void/personal_assistant/workspace-*`
    /// as SSH remote paths.
    ...
    /// Get Void-managed built-in skills directory under the user skills root.
    pub fn builtin_skills_dir(&self) -> PathBuf {
        self.user_skills_dir().join(".system")
    }

    /// Get cache root directory: ~/.config/void/cache/
    pub fn cache_root(&self) -> PathBuf {
        self.user_root.join("cache")
    }

    /// Get managed runtimes root directory: ~/.config/void/runtimes/
    ///
    /// Void-managed runtime components (e.g. node/python/office) are stored here.
    pub fn managed_runtimes_dir(&self) -> PathBuf {
        self.user_root.join("runtimes")
    }

    /// Get user data directory: ~/.config/void/data/
    ...
    pub fn remote_ssh_mirror_root() -> PathBuf {
        Self::new()
            .map(|pm| pm.void_home_dir().join("remote_ssh"))
            .unwrap_or_else(|_| {
                dirs::home_dir()
    ...
    /// Get scheduled jobs directory: ~/.config/void/data/cron/
    pub fn user_cron_dir(&self) -> PathBuf {
        self.user_data_dir().join("cron")
    }

    /// Get scheduled jobs persistence file: ~/.config/void/data/cron/jobs.json
    pub fn cron_jobs_file(&self) -> PathBuf {
        self.user_cron_dir().join("jobs.json")
    }

    /// Get miniapps root directory: ~/.config/void/data/miniapps/
    pub fn miniapps_dir(&self) -> PathBuf {
        self.user_data_dir().join("miniapps")
    }

    /// Get directory for a specific miniapp: ~/.config/void/data/miniapps/{app_id}/
    pub fn miniapp_dir(&self, app_id: &str) -> PathBuf {
        self.miniapps_dir().join(app_id)
    }

    /// Get user-level rules directory: ~/.config/void/data/rules/
    pub fn user_rules_dir(&self) -> PathBuf {
        self.user_data_dir().join("rules")
    }

    /// Get logs directory: ~/.config/void/logs/
    pub fn logs_dir(&self) -> PathBuf {
        self.user_root.join("logs")
    }

    /// Get temp directory: ~/.config/void/temp/
    ...
    pub fn projects_root(&self) -> PathBuf {
        self.void_home_dir().join("projects")
    }

    /// Get the runtime root for a workspace: ~/.void/projects/<workspace-slug>/
    pub fn project_runtime_root(&self, workspace_path: &Path) -> PathBuf {
        self.projects_root()
            .join(self.project_runtime_slug(workspace_path))
    }

    /// Get project internal config directory: {project}/.void/config/
    pub fn project_internal_config_dir(&self, workspace_path: &Path) -> PathBuf {
        self.project_root(workspace_path).join("config")
    }

    /// Get project agent profiles file: {project}/.void/config/agent_profiles.json
    ...
    pub async fn ensure_dir(&self, path: &Path) -> VoidResult<()> {
        if !path.exists() {
            tokio::fs::create_dir_all(path).await.map_err(|e| {
                VoidError::service(format!("Failed to create directory {:?}: {}", path, e))
            })?;
        }
        Ok(())
    }
    ...
    pub async fn initialize_user_directories(&self) -> VoidResult<()> {
        let dirs = vec![
            self.void_home_dir(),
            self.projects_root(),
...
fn init_global_path_manager() -> VoidResult<Arc<PathManager>> {
...
pub fn try_get_path_manager_arc() -> VoidResult<Arc<PathManager>> {
...
mod tests {
    ...
    fn assistant_workspace_paths_use_personal_assistant_subdir() {
        ...
        assert_eq!(
            base_dir,
            path_manager.void_home_dir().join("personal_assistant")
        );
        assert_eq!(
    ...
    #[test]
    fn legacy_assistant_workspace_paths_remain_at_void_root() {
        let path_manager = PathManager::default();
        let legacy_base_dir = path_manager.legacy_assistant_workspace_base_dir(None);

        assert_eq!(legacy_base_dir, path_manager.void_home_dir());
        assert_eq!(
    ...
    fn is_local_assistant_workspace_path_detects_personal_assistant_and_legacy() {
        ...
        assert!(!pm.is_local_assistant_workspace_path("/tmp/not-void"));
    }

    #[test]
    fn project_runtime_root_uses_human_readable_workspace_slug() {
        let pm = PathManager::default();
        let runtime_root = pm.project_runtime_root(Path::new(r"E:\Projects\OpenVoid\Void"));
