...
impl I18nService {
    ...
    pub async fn initialize(&self) -> VoidResult<()> {
    ...
    async fn load_all_bundles(&self) -> VoidResult<()> {
    ...
    pub async fn set_locale(&self, locale: LocaleId) -> VoidResult<()> {
        let old_locale = {
            let mut current = self.current_locale.write().await;
            let old = *current;
            *current = locale;
...
pub async fn sync_global_i18n_service_locale(locale: LocaleId) -> VoidResult<bool> {
