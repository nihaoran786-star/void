...
impl MCPConfigService {
    ...
    pub async fn load_mcp_json_config(&self) -> VoidResult<String> {
        match self
            .config_service
            ...
            .await
        {
            Ok(value) => format_mcp_json_config_value(Some(&value)).map_err(|e| {
                VoidError::serialization(format!("Failed to serialize MCP config: {}", e))
            }),
    ...
    pub async fn save_mcp_json_config(&self, json_config: &str) -> VoidResult<()> {
        ...
        let config_value: serde_json::Value = serde_json::from_str(json_config).map_err(|e| {
            ...
            error!("{}", error_msg);
            VoidError::validation(error_msg)
        })?;
