...
fn canonicalize_agent_profile(
    ...
    default_tools: &[String],
    valid_tools: &HashSet<String>,
) -> VoidResult<Option<AgentProfileConfig>> {
...
pub async fn get_agent_profile_configs() -> VoidResult<HashMap<String, AgentProfileConfig>> {
...
pub async fn get_agent_profile_views() -> VoidResult<HashMap<String, AgentProfileView>> {
...
pub async fn get_agent_profile_view(agent_id: &str) -> VoidResult<AgentProfileView> {
    let views = get_agent_profile_views().await?;
    views
        .get(agent_id)
        .cloned()
        .ok_or_else(|| VoidError::config(format!("Agent does not exist: {}", agent_id)))
}

pub async fn persist_agent_profile_from_value(agent_id: &str, config: Value) -> VoidResult<()> {
    let config_service = GlobalConfigManager::get_service().await?;
...
pub async fn reset_agent_profile_to_default(agent_id: &str) -> VoidResult<()> {
    let config_service = GlobalConfigManager::get_service().await?;
    let mut stored_configs = get_agent_profile_configs().await?;
    let profile_id = resolve_profile_id(agent_id);
...
) -> VoidResult<AgentProfileConfigCanonicalizationReport> {
    let config_service = GlobalConfigManager::get_service().await?;
    let valid_tools = get_valid_tool_names().await;
