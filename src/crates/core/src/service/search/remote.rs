...
fn resolve_local_flashgrep_bundle(binary_name: &str) -> Option<PathBuf> {
    let mut candidates = workspace_root_candidates()
        .into_iter()
        .map(|workspace_root| workspace_root.join("resources/flashgrep").join(binary_name))
        .collect::<Vec<_>>();
...
fn workspace_root_candidates() -> Vec<PathBuf> {
    ...
    if let Ok(cwd) = std::env::current_dir() {
        for path in cwd.ancestors() {
            if path.join("resources").join("flashgrep").exists() && seen.insert(path.to_path_buf())
            {
