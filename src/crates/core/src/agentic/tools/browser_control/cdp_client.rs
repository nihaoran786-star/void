...
impl CdpClient {
    ...
    pub async fn get_version(port: u16) -> VoidResult<CdpVersionInfo> {
        ...
        let info: CdpVersionInfo = resp
            .json()
            .await
            .map_err(|e| VoidError::tool(format!("Invalid CDP version response: {}", e)))?;
        Ok(info)
    }
    ...
    pub async fn list_pages(port: u16) -> VoidResult<Vec<CdpPageInfo>> {
        ...
        let pages: Vec<CdpPageInfo> = resp
            .json()
            .await
            .map_err(|e| VoidError::tool(format!("Invalid CDP pages response: {}", e)))?;
        Ok(pages)
    }

    /// Connect to a specific page by its WebSocket debugger URL.
    pub async fn connect(ws_url: &str) -> VoidResult<Self> {
        info!("CDP connecting to {}", ws_url);
        let (ws_stream, _) = connect_async(ws_url)
            .await
            .map_err(|e| VoidError::tool(format!("CDP WebSocket connect failed: {}", e)))?;
    ...
    pub async fn connect_to_first_page(port: u16) -> VoidResult<Self> {
        let pages = Self::list_pages(port).await?;
        let page = pages
            .iter()
            .find(|p| p.page_type.as_deref() == Some("page") && p.web_socket_debugger_url.is_some())
            .or_else(|| pages.first())
            .ok_or_else(|| VoidError::tool("No browser pages found via CDP".to_string()))?;

        let ws_url = page
            .web_socket_debugger_url
            .as_ref()
            .ok_or_else(|| VoidError::tool("Page has no WebSocket debugger URL".to_string()))?;

        Self::connect(ws_url).await
    }

    /// Send a CDP method call and wait for the response.
    pub async fn send(&self, method: &str, params: Option<Value>) -> VoidResult<Value> {
        ...
        debug!("CDP send id={} method={}", id, method);
        {
            ...
            sink.send(Message::Text(msg.to_string()))
                .await
                .map_err(|e| VoidError::tool(format!("CDP send failed: {}", e)))?;
        }

        let result = tokio::time::timeout(std::time::Duration::from_secs(30), rx)
            .await
            .map_err(|_| VoidError::tool(format!("CDP timeout for method {}", method)))?
            .map_err(|_| VoidError::tool("CDP response channel closed".to_string()))?;

        if let Some(error) = result.get("error") {
            return Err(VoidError::tool(format!("CDP error: {}", error)));
        }
