...
async fn persist_tool_result(
    ...
    preview_chars: usize,
    content_override: Option<String>,
) -> VoidResult<PersistedToolOutput> {
    let session_id = context.session_id.as_deref().ok_or_else(|| {
        VoidError::tool("A session id is required to persist tool results".to_string())
    })?;
...
async fn write_once(path: &Path, content: &str) -> VoidResult<()> {
...
fn serialize_tool_result_content(result: &ToolResult) -> VoidResult<(String, bool)> {
    ...
    serde_json::to_string_pretty(&result.result)
        ...
        .map(|text| (text, true))
        .map_err(|error| {
            VoidError::serialization(format!("Failed to serialize tool result: {}", error))
        })
}
