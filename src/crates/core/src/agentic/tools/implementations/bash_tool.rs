...
impl BashTool {
    ...
    pub fn noninteractive_env() -> std::collections::HashMap<String, String> {
        ...
        env.insert("VOID_NONINTERACTIVE".to_string(), "1".to_string());
    ...
    fn cancellation_error(stage: &str) -> VoidError {
        VoidError::cancelled(format!("Bash tool execution cancelled {}", stage))
    }
  ...
  - Try to maintain your current working directory throughout the session by using absolute paths and avoiding usage of `cd`. You may use `cd` if the User explicitly requests it.
    ...
    async fn description_with_context(
        ...
        if context.map(|c| c.is_remote()).unwrap_or(false) {
            base = format!(
                r#"**Remote workspace:** Commands run on the **SSH server** in a shell whose initial working directory is the **remote workspace root** (same as running a terminal on that machine). The shell name shown below may reflect your **local** Void settings; the actual interpreter on the server is typically `sh`/`bash`. Use **Unix** syntax and POSIX paths — not PowerShell or Windows paths.
...
{base}"#,
    ...
    async fn call(&self, input: &Value, context: &ToolUseContext) -> VoidResult<Vec<ToolResult>> {
        ...
        let chat_session_id = context
            .session_id
            .as_ref()
            .ok_or_else(|| VoidError::tool("session_id is required for Bash tool".to_string()))?;
        ...
        let terminal_api = TerminalApi::from_singleton()
            .map_err(|e| VoidError::tool(format!("Terminal not initialized: {}", e)))?;
        ...
        let primary_session_id = binding
            ...
            .await
            .map_err(|e| VoidError::tool(format!("Failed to create Terminal session: {}", e)))?;
...
impl BashTool {
    ...
    async fn call_background(
        ...
        let output_file = tokio::fs::OpenOptions::new()
            ...
            .await
            .map_err(|e| {
                VoidError::tool(format!("Failed to open background output file: {}", e))
            })?;
