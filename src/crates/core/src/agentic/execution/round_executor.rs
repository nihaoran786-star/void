...
impl RoundExecutor {
    ...
    pub async fn execute_round(
        ...
        if let Some(ref usage) = stream_result.usage {
            ...
            self.emit_event(
                AgenticEvent::TokenUsageUpdated {
                    ...
                    prompt_cache_telemetry: prompt_cache_telemetry_from_usage(usage),
                },
        ...
        self.emit_event(
            AgenticEvent::ModelRoundCompleted {
                ...
                prompt_cache_telemetry: stream_result
                    .usage
                    .as_ref()
                    .and_then(prompt_cache_telemetry_from_usage),
            },
            EventPriority::High,
        )
        .await;
        ...
        for idx in &write_indices {
            ...
            let content_prompt = format!(
                "Now output the COMPLETE file content for the file `{file_path}`.\n\
                 ...
                 9. Do NOT output `[called tools:` markers, tool parameter JSON, or `<void_contents>` / `</void_contents>` tags — the opening tag is already provided via prefill. Begin with the first byte of the file content immediately after that opening tag.",
                file_path = file_path
            );
            ...
            let content = {
                ...
                loop {
                    ...
                    let extracted = extract_void_contents_with_options(&full_text, true);
                        ...
                        if content_attempt + 1 >= Self::MAX_WRITE_CONTENT_QUALITY_ATTEMPTS {
                            let error = format!(
                                "Write content generation produced no file content for {} after {} attempts. \
                                 The follow-up request returned an empty <void_contents> body \
    ...
    fn build_write_content_messages(
        ...
        content_messages.push(AIMessage::assistant("<void_contents>".to_string()));
        content_messages
    }
            ...
            loop {
                ...
                match chunk {
                    Ok(resp) => {
                        ...
                        let preview_content = extract_void_contents_with_options(&text, true);
...
fn prompt_cache_telemetry_from_usage(
    usage: &crate::util::types::ai::GeminiUsage,
) -> Option<serde_json::Value> {
    let telemetry = void_runtime_ports::PromptCacheTelemetry::from_provider_counts(
        usage.prompt_token_count as usize,
        usage.cached_content_token_count.map(|value| value as usize),
        usage.cache_creation_token_count.map(|value| value as usize),
    );

    serde_json::to_value(telemetry).ok()
}
...
fn extract_void_contents(text: &str) -> String {
    extract_void_contents_with_options(text, false)
}

fn extract_void_contents_with_options(text: &str, prefill_open_tag: bool) -> String {
...
fn strip_void_content_tags(content: &str) -> String {
    content
        .replace("<void_contents>", "")
        .replace("</void_contents>", "")
}
...
mod tests {
    ...
    fn write_content_messages_preserve_full_conversation_prefix() {
        ...
        use void_ai_adapters::types::{Message as AIMessage, ToolCall};
        ...
        assert_eq!(messages[4].content.as_deref(), Some("<void_contents>"));
        assert!(messages[4]
            .content
            .as_deref()
    ...
    #[test]
    fn extract_void_contents_prefilled_stream_without_open_tag() {
        let text = "# Title\n\nBody paragraph.\n";
        assert_eq!(
            extract_void_contents_with_options(text, true),
            "# Title\n\nBody paragraph."
        );
    }

    #[test]
    fn extract_void_contents_prefilled_stream_strips_called_tools_preamble() {
        let text = concat!(
    ...
    #[test]
    fn extract_void_contents_prefilled_stream_uses_last_open_tag() {
        let text = concat!(
            "[called tools: Read with params: {\"file_path\":\"a.md\"}]",
            "<void_contents>\n# Wrong\n",
            "<void_contents>\n# Correct\n"
        );
        assert_eq!(extract_void_contents_with_options(text, true), "# Correct");
    }
    ...
    #[test]
    fn prompt_cache_telemetry_reports_provider_partial_hit() {
        use crate::util::types::ai::GeminiUsage;
        let usage = GeminiUsage {
            prompt_token_count: 100,
            candidates_token_count: 20,
        ...
        let telemetry =
            super::prompt_cache_telemetry_from_usage(&usage).expect("prompt cache telemetry");

        assert_eq!(telemetry["eligible"], true);
        assert_eq!(telemetry["status"], "partial_hit");
        assert_eq!(telemetry["source"], "provider");
        assert_eq!(telemetry["cachedTokens"], 40);
    }

    #[test]
    fn prompt_cache_telemetry_reports_unavailable_when_provider_omits_cache_fields() {
        use crate::util::types::ai::GeminiUsage;
        let usage = GeminiUsage {
            prompt_token_count: 100,
            candidates_token_count: 20,
        ...
        assert_eq!(telemetry["eligible"], false);
        assert_eq!(telemetry["status"], "unavailable");
        assert_eq!(telemetry["source"], "unavailable");
    }
}
