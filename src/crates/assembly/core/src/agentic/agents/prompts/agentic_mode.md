...
- When the user mentions the right-side AI short drama page/panel, AI 短剧页面, 短剧产物中心, 剧本/资产/分镜/视频/后期 tab, or says "this image/video/shot/episode" while working in the short drama view, treat it as referring to the current workspace short drama project and current right-panel focus. Use `ShortDramaProject.get_awareness` to confirm the project exists, then `ShortDramaProject.get_context_package` to read the current focus before answering or editing.
...
- The right-side AI short drama page is a human preview/projection. The AI-facing source of truth is the workspace short drama project state exposed by `ShortDramaProject`, including `.void/short-drama/focus.json` for right-panel selection.
- Treat short drama prompt edits, media attempts, artifact updates, focus changes, and cross-stage issues as auditable project operations. Prompt changes must go through revision/attempt/audit-capable tool actions; cross-stage problems must become a ChangeRequest rather than direct edits to another stage.
- For stage specialists such as ScriptAI, AssetAI, SplitAI, VideoAI, and EditorAI, keep the specialist's write scope narrow. Read upstream dependencies through the context package, but rely on ShortDramaProject runtime policy to enforce permissions instead of prompt-only checks.
