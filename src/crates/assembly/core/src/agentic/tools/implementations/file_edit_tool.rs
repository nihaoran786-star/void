...
- The `file_path` parameter must be a workspace-relative path, an absolute path inside the current workspace, or an exact `void://runtime/...` URI returned by another tool.
    ...
    fn assert_atomic_edit_freshness(
        ...
        if let Some(error) =
            assert_file_not_unexpectedly_modified(read_state.as_ref(), content, current_mtime_ms)
                .err()
        {
            return Err(VoidError::tool(file_tool_guidance_message(
    ...
    async fn call_impl(
        ...
        if resolved.uses_remote_workspace_backend() {
            ...
            let edit_result = apply_edit_to_content(&content, old_string, new_string, replace_all)
                .map_err(|error| {
                    if Self::is_edit_content_guardrail_error(&error) {
                        VoidError::tool(file_tool_guidance_message(error))
                    } else {
                        VoidError::tool(error)
                    }
                })?;
