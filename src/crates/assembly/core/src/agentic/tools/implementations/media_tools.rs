...
fn optional_short_drama_metadata(input: &Value) -> Option<Value> {
    input
        .get("short_drama")
        .or_else(|| input.get("shortDrama"))
...
fn normalize_aspect_ratio_text(value: Option<&str>, fallback: &'static str) -> String {
    let Some(value) = value else {
        return fallback.to_string();
    };
    let trimmed = value.trim();
    if trimmed.eq_ignore_ascii_case("auto") || trimmed.eq_ignore_ascii_case("adaptive") {
        return fallback.to_string();
    }
    let Some((width, height)) = trimmed.split_once(':') else {
        return fallback.to_string();
    };
...
fn placeholder_aspect_ratio(value: &str) -> String {
    value.replace(':', " / ")
}
...
mod media_image_reference_tests {
    use super::{
        image_generation_request_from_input, normalize_aspect_ratio_text, placeholder_aspect_ratio,
        resolve_media_image_urls, resolve_media_upload_path_reference, upload_image_result,
        video_request_from_input, GenerateImageTool,
    };
    ...
    #[test]
    fn normalizes_placeholder_aspect_ratio_metadata() {
        assert_eq!(normalize_aspect_ratio_text(Some("9:16"), "1:1"), "9:16");
        assert_eq!(normalize_aspect_ratio_text(Some("auto"), "1:1"), "1:1");
        assert_eq!(
            normalize_aspect_ratio_text(Some("adaptive"), "16:9"),
            "16:9"
        );
        assert_eq!(placeholder_aspect_ratio("21:9"), "21 / 9");
        assert_eq!(
            video_request_from_input(&json!({ "aspect_ratio": "21:9", "size": "16:9" }))
                .aspect_ratio,
            Some("21:9".to_string())
        );
    }
