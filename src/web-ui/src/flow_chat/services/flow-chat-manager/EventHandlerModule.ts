...
const SUBAGENT_SESSION_LINKED_OBSERVER_EVENT = 'agent:subagent-session-linked';
...
export interface SubagentSessionLinkedObserverEvent {
...
export interface AgentToolRunObserverEvent {
  sessionId: string;
  turnId?: string;
  roundId?: string;
  eventType: FlowToolEvent['event_type'];
  toolId: string;
  toolName: string;
  params?: unknown;
  result?: unknown;
...
export function emitSubagentSessionLinkedEventForObservers(
  rawEvent: Partial<SubagentSessionLinkedEvent> & Record<string, unknown>,
): boolean {
  const childSessionId = rawEvent.sessionId ?? rawEvent.childSessionId;
  const parentSessionId = rawEvent.parentSessionId ?? rawEvent.parent_session_id;
  if (typeof childSessionId !== 'string' || typeof parentSessionId !== 'string') {
    return false;
  }

  const parentDialogTurnId = rawEvent.parentDialogTurnId ?? rawEvent.parent_dialog_turn_id;
  const parentToolCallId = rawEvent.parentToolCallId ?? rawEvent.parent_tool_call_id;
  const agentType = rawEvent.agentType ?? rawEvent.agent_type;
  const workspacePath = rawEvent.workspacePath ?? rawEvent.workspace_path;

  return globalEventBus.emit<SubagentSessionLinkedObserverEvent>(SUBAGENT_SESSION_LINKED_OBSERVER_EVENT, {
    childSessionId,
    parentSessionId,
    parentDialogTurnId: typeof parentDialogTurnId === 'string' ? parentDialogTurnId : undefined,
...
export function emitAgentToolRunEventForObservers(event: {
  sessionId: string;
  turnId?: string;
  roundId?: string;
  toolEvent: FlowToolEvent;
  ...
  return globalEventBus.emit<AgentToolRunObserverEvent>(AGENT_TOOL_RUN_OBSERVER_EVENT, {
    sessionId,
    turnId,
    roundId,
    ...
    params: 'params' in toolEvent ? toolEvent.params : undefined,
    result: 'result' in toolEvent ? toolEvent.result : undefined,
    error: 'error' in toolEvent ? toolEvent.error : undefined,
    reason: 'reason' in toolEvent ? toolEvent.reason : undefined,
  }, 'EventHandlerModule');
}
...
function handleToolEvent(
  ...
  emitAgentToolRunEventForObservers({ sessionId, turnId, roundId, toolEvent });
  clearRuntimeStatus(context, sessionId, turnId);
  touchPendingTurnCompletion(context, sessionId, turnId);
...
function handleModelRoundComplete(context: FlowChatContext, event: ModelRoundCompletedEvent): void {
  ...
  context.flowChatStore.updateModelRound(sessionId, turnId, roundId, current => ({
    ...
    promptCacheTelemetry:
      event.promptCacheTelemetry ?? (event as any).prompt_cache_telemetry,
  }));
