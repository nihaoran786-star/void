...
export class FlowChatManager {
  ...
  async ensureBackendSession(sessionId: string): Promise<void> {
    return ensureBackendSessionModule(this.context, sessionId);
  }
  ...
  private handleTodoWriteResult(sessionId: string, turnId: string, result: any): void {
    try {
      ...
      window.dispatchEvent(new CustomEvent('void:todowrite-update', {
        detail: {
          sessionId,
          turnId,
          todos: incomingTodos,
