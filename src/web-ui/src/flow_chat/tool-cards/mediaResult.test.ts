...
describe('getMediaToolViewModel', () => {
  ...
  it('exposes backend aspect ratio metadata from polling image batches', () => {
    const model = getMediaToolViewModel(toolWithResult({
      status: 'polling',
      source: 'apimart',
      kind: 'image',
      batch: {
        batch_id: 'media-batch-aspect',
        kind: 'image',
        status: 'polling',
        total_count: 1,
        completed_count: 0,
    ...
    expect(model?.requestedAspectRatio).toBe('9:16');
    expect(model?.placeholderAspectRatio).toBe('9 / 16');
  });
