...
export class FlowChatStore {
  ...
  private clearOldStorage(): void {
    ...
    try {
      ...
      Object.keys(localStorage).forEach(key => {
        if (key.startsWith('void-session-')) {
          localStorage.removeItem(key);
        }
      });
    } catch (error) {
  ...
  public addExternalSession(
    ...
    this.setState(prev => {
      ...
      const session: Session = {
        ...
        historyState: meta?.historyState ?? 'new',
        workspacePath,
        remoteConnectionId,
        remoteSshHost,
        parentSessionId: relationship.parentSessionId,
  ...
  public switchSession(sessionId: string): void {
    ...
    window.dispatchEvent(new CustomEvent('void:session-switched', {
  ...
  public cancelSessionTask(sessionId: string): void {
    this.setState(prev => {
      ...
      window.dispatchEvent(new CustomEvent('void:dialog-cancelled', {
        detail: { sessionId }
      }));
      ...
      const turnData = {
        ...
        modelRounds: dialogTurn.modelRounds.map((round, roundIndex) => {
          ...
          return {
            ...
            promptCacheTelemetry: round.promptCacheTelemetry,
            status: round.status
          };
        }),
