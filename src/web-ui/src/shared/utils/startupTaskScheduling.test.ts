...
describe('scheduleAfterStartupPaint', () => {
  ...
  it('waits for the startup signal before scheduling paint-delayed work', () => {
    ...
    expect(addEventListener).toHaveBeenCalledWith('void:main-window-shown', expect.any(Function));
    ...
    expect(task).toHaveBeenCalledTimes(1);
    expect(removeEventListener).toHaveBeenCalledWith('void:main-window-shown', expect.any(Function));
  });
