...
export function isvoidRuntimeUri(path: string): boolean {
  return typeof path === 'string' && path.startsWith('void://runtime/');
}
 ...
 * skips URI percent-decoding: the input is an OS-native path, not a URI, so
 * file names containing literal `%XX` sequences must be preserved. UNC paths
 * (`\\?\`, `\\server\...`) are returned as-is so their backslashes are not
 * turned into slashes.
 */
