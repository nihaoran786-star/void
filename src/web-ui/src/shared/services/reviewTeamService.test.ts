...
describe('reviewTeamService', () => {
  ...
  it('adds a locale-only guardrail for i18n-only frontend review targets', () => {
    ...
    expect(promptBlock).toContain('Void-Installer/src/i18n/locales/zh-TW.json');
    expect(promptBlock).not.toContain('Void-Installer/src/i18n/Void-Installer/src/i18n/locales/zh-TW.json');
  });
  ...
  it('builds a metadata-only evidence pack without source, diff, or model output', () => {
    ...
    const files = [
      ...
      'src/apps/desktop/src/api/agentic_api.rs',
      'src/crates/assembly/api-layer/src/review.rs',
      'package.json',
    ];
    ...
    expect(manifest.evidencePack?.contractHints).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          kind: 'api_contract',
          filePath: 'src/crates/assembly/api-layer/src/review.rs',
        }),
        expect.objectContaining({
          kind: 'config_key',
