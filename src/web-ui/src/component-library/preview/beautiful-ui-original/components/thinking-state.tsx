...
function useSequence(steps: number[], disabled = false) {
  const [stage, setStage] = useState(0);
  useEffect(() => {
    if (disabled || stage >= steps.length - 1) return;
...
export default function ThinkingState({
  ...
  const renderedRows = rows ?? v.rows;
  const autoExpanded = stage >= 1 && stage < 4;
  const expanded = alwaysExpanded || (manualExpanded ?? autoExpanded);
  const working = controlledWorking ?? stage < 3;
  ...
  return (
    <div key={variant} className={`flex w-full max-w-95 flex-col ${compact ? "" : "min-h-[176px]"}`}>
      {/* header — shared across variants */}
      {alwaysExpanded ? (
        <div className="-mx-1.5 flex w-fit items-center gap-2 px-1.5 py-1" aria-live={working ? "polite" : undefined}>
          {headerContent}
        </div>
      ) : (
        <button
          ...
          <div className="relative mt-1 ml-[5px] pl-4">
            ...
            {(query ?? v.query) && (
              <div className="flex h-6 items-center gap-2 px-1.5" style={{ animation: expanded ? "fade-up 300ms cubic-bezier(0.23,1,0.32,1) both" : undefined }}>
                ...
                <span className="text-[12.5px] text-ink-2">{query ?? v.query}</span>
              </div>
            )}
            {renderedRows.slice(0, visible).map((row, i) => {
              const content = (
                <>
