...
const LEGACY_REMOVED_PET_TOKEN = ['pan', 'da'].join('');

function normalizePresetPet(pet: AgentCompanionPetSelection): AgentCompanionPetSelection {
  const searchable = [
    pet.id,
    pet.displayName,
    pet.packagePath,
  ...
  return searchable.includes(LEGACY_REMOVED_PET_TOKEN)
...
async function withPreviewSrc(pet: AgentCompanionPetSelection): Promise<AgentCompanionPetPackage> {
  const normalizedPet = pet.source === 'preset' ? normalizePresetPet(pet) : pet;
  const previewSrc = normalizedPet.source === 'preset'
    ? normalizedPet.spritesheetPath
    : isTauriRuntime()
      ? await readFileAsBlobUrl(normalizedPet.spritesheetPath, normalizedPet.spritesheetMimeType)
      : '';
  return { ...normalizedPet, previewSrc };
}
...
export async function resolveAgentCompanionPetSrc(
  ...
  if (!pet) return '';
  const normalizedPet = pet.source === 'preset' ? normalizePresetPet(pet) : pet;
  if (normalizedPet.source === 'preset') return normalizedPet.spritesheetPath;
  if (!isTauriRuntime()) return '';
  return readFileAsBlobUrl(normalizedPet.spritesheetPath, normalizedPet.spritesheetMimeType);
}
