...
export const ImageViewer: React.FC<ImageViewerProps> = ({
  ...
  return (
    <div className={`void-image-viewer ${className} ${isFullscreen ? 'fullscreen' : ''}`}>
      <div className="void-image-viewer__toolbar">
        <div className="void-image-viewer__info">
          <span className="void-image-viewer__filename">{fileName || filePath.split(/[/\\]/).pop()}</span>
          {imageDimensions && (
            <span className="void-image-viewer__dimensions">
              {imageDimensions.width} × {imageDimensions.height}
            </span>
          )}
          {fileSize > 0 && (
            <span className="void-image-viewer__filesize">
              {formatFileSize(fileSize)}
            </span>
          )}
        </div>
        <div className="void-image-viewer__controls">
          <Tooltip content={t('editor.imageViewer.zoomOut')} placement="top">
            <button
              className="void-image-viewer__btn"
          ...
          <Tooltip content={t('editor.imageViewer.zoomReset')} placement="top">
            <button
              className="void-image-viewer__btn void-image-viewer__btn--zoom-display"
          ...
          <div className="void-image-viewer__divider" />
      ...
      <div className="void-image-viewer__container">
        {loading && (
          <div className="void-image-viewer__loading">
            <div className="void-image-viewer__spinner" />
            <p>{t('editor.common.loading')}</p>
          </div>
        )}

        {error && (
          <div className="void-image-viewer__error">
            <p>{error}</p>
            <p className="void-image-viewer__error-path">{filePath}</p>
          </div>
        )}

        {!loading && !error && imageUrl && (
          <div className="void-image-viewer__image-wrapper">
            <img
              src={imageUrl}
              alt={fileName || filePath}
              className="void-image-viewer__image"
              style={{
